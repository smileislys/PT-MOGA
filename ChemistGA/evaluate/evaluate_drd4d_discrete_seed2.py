#!/usr/bin/env python3
"""Evaluate four-objective discrete Retro*-solved molecules, seed 2.

Synthesizability is calculated over all 5,000 tested entries.
Novelty, diversity and scaffold count are calculated on all Retro*-solved
perfect-molecule entries without removing duplicate molecules.
"""
...
def load_retro_results(path):
    ...
    # Retain the latest record if a failed entry was rerun.
...
def main():
    ...
    if solved_count_smi != solved_count_csv:
        raise RuntimeError(
            ...
            f"{solved_count_smi} entries, but the Retro* "
            f"CSV records {solved_count_csv} solved entries."
        )
    ...
    print("=" * 72)
    print(
        "PT-MOGA four-objective discrete evaluation | seed 2"
    )
    print("=" * 72)
