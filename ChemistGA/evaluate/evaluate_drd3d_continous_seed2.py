#!/usr/bin/env python3
"""Evaluate DRD2 3D continuous Retro*-solved molecules, seed 2."""
...
REFERENCE_FILE = os.path.join(
    BASE_PATH, "data", "inh", "drd_succ_250.csv"
)
...
RETRO_RESULT_FILE = os.path.join(
    ...
    "output",
    "retro_recorded_3d",
    f"drd_continuous_retro_results_seed{SEED}.csv",
)
...
SOLVED_SMI_FILE = os.path.join(
    ...
    "output",
    "retro_recorded_3d",
    f"drd_continuous_retro_solved_seed{SEED}.smi",
)
...
OUTPUT_FILE = os.path.join(
    ...
    "output",
    "retro_recorded_3d",
    f"drd_continuous_perfect_metrics_seed{SEED}.csv",
)
...
def load_retro_results(path):
    ...
    required_columns = {"input_index", "SMILES", "solved", "error"}
    ...
    if error_mask.any():
        raise RuntimeError(
            ...
            f"planner errors. Rerun them before evaluation."
        )
...
def load_solved_molecules(path):
    ...
    for smiles in smiles_list:
        ...
        if mol is None:
            raise RuntimeError(
                f"Invalid solved SMILES encountered: {smiles}"
            )

        # 不去重，每一条 solved 记录都保留
        molecules.append(mol)
        canonical_smiles.append(canonical)

    duplicate_count = (
        len(canonical_smiles) - len(set(canonical_smiles))
    )
...
def main():
    ...
    retro_results = load_retro_results(
        RETRO_RESULT_FILE
    )
    ...
    ) = load_solved_molecules(SOLVED_SMI_FILE)
    ...
    tested_count = len(retro_results)
    solved_count_csv = int(retro_results["solved"].sum())
    solved_count_smi = len(perfect_molecules)

    if solved_count_smi != solved_count_csv:
        raise RuntimeError(
            f"Solved SMI contains {solved_count_smi} entries, "
            f"but Retro* CSV records {solved_count_csv} solved entries."
        )
    ...
    metrics = {
        "seed": SEED,
        "tested_success_molecule_entries": tested_count,
        "retro_solved_perfect_entries": solved_count_smi,
        "synthesizability_percent":
            100.0 * solved_count_smi / tested_count,
        ...
        "diversity_all_perfect_entries":
            calculate_diversity(predicted_fps),
        "unique_bemis_murcko_scaffolds":
            count_scaffolds(perfect_molecules),
    }
    ...
    print("=" * 72)
    print(
        "PT-MOGA DRD2 three-objective continuous evaluation | seed 2"
    )
    print("=" * 72)
    ...
    print(
        ...
        % (
            metrics["retro_solved_perfect_entries"],
            metrics["tested_success_molecule_entries"],
            metrics["synthesizability_percent"],
        )
    )

    print(
        "Solved entries evaluated without deduplication: %d"
        % metrics["retro_solved_perfect_entries"]
    )
    ...
    print(
        ...
        % (
            metrics["canonical_duplicate_entries_not_removed"],
            metrics[
                "canonical_unique_perfect_molecules_information_only"
            ],
        )
