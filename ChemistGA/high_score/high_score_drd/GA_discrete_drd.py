...
def calculate_3d_scores(population):
    """
    🔥 全新裁判系统：单靶点 0/1 离散化打分版 (Discretized Fitness) 🔥
    只要及格，就是满分 (1)！没及格就是 0！
    彻底消除绝对高分的压制，迫使帕累托拥挤度算法去寻找结构最奇特的分子！
    """
    # 1. 召唤三个裁判的实体
    ...
    for i in range(len(population)):
        ...
        # DRD2 及格线: 0.5
        drd2_disc = 1.0 if float(drd2_list[i]) >= 0.5 else 0.0

        # QED 及格线: 0.6
        qed_disc = 1.0 if float(qed_list[i]) >= 0.6 else 0.0

        # SA 及格线: 4.0 (注意，SA 原始值是越小越好，小于4.0给1分)
        sa_disc = 1.0 if float(sa_list[i]) <= 4.0 else 0.0

        # 组装这名选手的 3 维雷达图数据（传给帕累托的全是 0 和 1）
        scores_3d.append([drd2_disc, qed_disc, sa_disc])
...
# ✨ 修复：自动创建离散版专属输出文件夹，防止覆盖连续型数据
# 👉 修改为：
out_dir_top3 = os.path.join(base_path, 'output', 'drd_top3_discrete')
out_dir_all = os.path.join(base_path, 'output', 'drd_all_pop_discrete')
