#!/usr/bin/env python3
"""Run the four-objective continuous Retro* benchmark.

The input must contain 5,000 generated molecules satisfying the
JNK3/GSK3β/QED/SA property criteria. The script records the Retro*
result for every input SMILES and supports checkpoint resumption.

Default input:
    output/continuous_for_retro_test_5000_seed1.smi
"""
...
DEFAULT_TASK_PREFIX = "continuous"
...
def parse_args():
    ...
    # Four-objective continuous seed 1
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="+",
...
def plan_single_smi(smi):
    """Return the Retro* solved flag and preserve planner errors."""
    ...
    try:
        ...
        # Compatibility with the return convention used by the old code
        return 1, ""
...
def load_previous_results(path):
    ...
    if not required.issubset(frame.columns):
        raise RuntimeError(
            "Existing checkpoint has incompatible columns: %s"
            % path
        )
...
def finalize_seed(
    ...
    # A rerun may append a corrected result for an existing index
    ...
    if missing:
        raise RuntimeError(
            "Seed %d is incomplete: %d molecules have no "
            "recorded result. Rerun to resume."
            % (seed, len(missing))
        )

    final_csv = os.path.join(
        output_dir,
        "%s_retro_results_seed%d.csv"
        % (task_prefix, seed),
    )
    ...
    solved_path = os.path.join(
        output_dir,
        "%s_retro_solved_seed%d.smi"
        % (task_prefix, seed),
    )

    unsolved_path = os.path.join(
        output_dir,
        "%s_retro_unsolved_seed%d.smi"
        % (task_prefix, seed),
    )
    ...
    rate = (
        100.0 * solved_count / len(smiles_list)
        if smiles_list
        else 0.0
    )
...
def run_seed(
    ...
    input_path = os.path.join(
        base_path,
        "output",
        "%s_for_retro_test_5000_seed%d.smi"
        % (task_prefix, seed),
    )
    ...
    if len(smiles_list) != EXPECTED_MOLECULES:
        raise RuntimeError(
            "%s contains %d molecules; the benchmark "
            "requires exactly %d."
            % (
                input_path,
                len(smiles_list),
    ...
    if duplicate_count:
        print(
            "WARNING: seed %d input contains %d duplicate "
            "SMILES entries. They will not be removed."
    ...
    write_header = (
        not os.path.exists(raw_result_path)
        or os.path.getsize(raw_result_path) == 0
    )
...
def main():
    ...
    # Four-objective task output directory
    ...
    for seed in args.seeds:
        ...
        print(
            "Four-objective continuous | seed %d"
            % seed,
            flush=True,
        )
    ...
    if len(summary_frame) == 1:
        ...
        print(
            ...
            % (
                ...
                float(
                    row["synthesizability_percent"]
                ),
            )
        )
    else:
        print(
            "Mean synthesizability: %.2f +/- %.2f%%"
            % (
                np.mean(rates),
                np.std(rates),
            )
    ...
    if int(
        summary_frame["planner_errors"].sum()
    ) > 0:
        print(
            "WARNING: planner errors were recorded. "
            "Inspect and rerun them before reporting."
        )
