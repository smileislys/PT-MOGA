...
# 👇 核心：精准指向 NSGA-III 4维混合大池子
output_dir = os.path.join(base_path, "output", "jnk_gsk_nsga3_4d_all")

print("⏳ 正在加载 4 维模型裁判 (JNK3, GSK3, QED, SA)...")
...
for seed in range(3):
    print(f"\n" + "█" * 60)
    print(f"🚀 正在提纯: Seed {seed} (NSGA-III 4目标 - 抽取 500 个) 🚀")
    print("█" * 60)

    all_files = glob.glob(f"{output_dir}/{seed}_*_every_all.csv")
    if not all_files:
        print(f"⚠️ 找不到 Seed {seed} 的 _all 文件，请跳过。")
        continue
    ...
    # 👇 4 维严苛过滤 (JNK3>=0.5, GSK3>=0.5, QED>0.6, SA<=4.0)
    for i in range(len(candidate_smiles)):
        if float(jnk3_scores[i]) >= 0.5 and float(gsk3_scores[i]) >= 0.5 and float(qed_scores[i]) > 0.6 and float(
                sa_scores[i]) <= 4.0:
            mol = Chem.MolFromSmiles(candidate_smiles[i])
            if mol is not None:
                valid_smiles.append(candidate_smiles[i])

    print(f"   ✅ Seed {seed} 最终幸存【4维及格分子】数: {len(valid_smiles)}")
    ...
    # 保存专属命名的 .smi 文件
    output_smi_file = os.path.join(base_path, "output", f"nsga3_4d_for_retro_test_500_seed{seed}.smi")
