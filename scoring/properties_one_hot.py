...
class gsk3_model():
    ...
    def __init__(self):
        # 换成稳如泰山的绝对路径
        self.clf_path = '/home/liuyansong/ChemistGA-master/ChemistGA-master/data/gsk3/gsk3.pkl'
    ...
    def __call__(self, smiles_list):
        ...
        for i, smiles in enumerate(smiles_list):
            ...
            # [新增] 如果这批分子全军覆没，直接给所有人打 0 分
            # 把 len(data) 改成真正的参数名，比如 len(smiles_list)
        if len(fps) == 0:
...
class jnk3_model():
    ...
    def __init__(self):
        # 换成稳如泰山的绝对路径
        self.clf_path = '/home/liuyansong/ChemistGA-master/ChemistGA-master/data/jnk3/jnk3.pkl'
        with open(self.clf_path, "rb") as f:
            self.clf = pickle.load(f)
    ...
    def __call__(self, smiles_list):
        ...
        fps = np.concatenate(fps, axis=0)  # 这是原本那行
        scores = self.clf.predict_proba(fps)[:, 1]
        scores = scores * np.array(mask)
        return np.float32(scores)
